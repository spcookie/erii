...
erii 是一个具有情感、记忆、心流和主动行为的 AI 群聊机器人。erii 能够像真人一样参与群聊对话，主动插话、破冰，并根据情绪状态调整说话风格。同时还支持
**事项提醒**和**自我规则管理**，可通过文件系统管理行为规则。
...
- **Env Config** - 环境变量配置（`.env.local`）
- **Application Config** - 应用配置（`application.conf`）
- **Plugin Config** - 插件配置管理
- **Souls** - 灵魂配置浏览
- **Rules** - 规则文件浏览

使用上下键导航，Enter 进入编辑，Esc 返回。
...
│       └── processor/           # 注解处理器
├── erii-plugins/                # 🎨 插件模块 (Git Submodule → spcookie/erii-plugins)
...
├── erii-cli/                    # 🖥️ CLI 工具模块
│   ├── cmd/                    # CLI 命令
│   │   └── config.go           # config 命令 - TUI 配置管理
│   └── internal/tui/           # TUI 界面组件
│       ├── components/         # TUI 组件
│       │   ├── config/         # 配置编辑器
│       │   ├── md/             # Markdown 浏览器
│       │   └── plugin/         # 插件浏览器
