...
[🚀 Features](#-features) • [🏁 Getting Started](#-getting-started) • [🛠️ Tech Stack](#-tech-stack) • [📦 Building \& Running](#-building--running)
...
2. **配置环境变量**
   创建 `.uesugi/.env.local` 文件并配置以下变量：
   ```properties
   # LLM API 配置
   GOOGLE_API_KEY=your_google_api_key_here
   DEEP_SEEK_API_KEY=your_deep_seek_api_key_here
   MINIMAX_CODING_PLAN_KEY=your_minimax_api_key_here
   CHOICE_MODEL=MINIMAX  # GOOGLE / DEEP_SEEK / MINIMAX

   # NapCat Token（与 onebot.ws 对应）
   NAPCAT_TOKEN=your_napcat_token

   # Embedding 配置（可选，用于记忆系统）
   EMBEDDING_API_KEY=your_bytedance_embedding_key_here

   # 搜索配置（可选，用于 QA 插件）
   SEARCH_API_KEY=your_exa_search_key_here

   # 浏览器服务（可选，用于网页截图/Markdown 抓取）
   PLAYWRIGHT_HOST=ws://127.0.0.1:13001

   # 代理配置（可选）
   HTTP_PROXY=http://127.0.0.1:7899
   SOCKS_PROXY=http://127.0.0.1:7898

   # NapCat UID/GID（Docker 部署时使用）
   NAPCAT_UID=your_uid
   NAPCAT_GID=your_gid
   ```

3. **配置机器人**
   编辑 `.uesugi/application.conf` 中的 `onebot.bots` 和 `groups` 配置：
   ```properties
   onebot {
     bots = {
       your_bot_name = {
         ...
         token = ${NAPCAT_TOKEN}
         role-id = "YourBotRole"    # 对应的角色 ID
       }
     }
   }

   groups {
     debug-group-id = null           # 调试群组（可选）
     enable-groups = [123456789, 987654321]  # 启用的群组列表
     message-redirect-map = []       # 消息重定向映射
   }
   ```
...
这将构建并启动 Erii 服务。如需单独启动各个服务：

```bash
docker-compose up -d napcat_0    # NapCat QQ 接入服务（Bot 1）
docker-compose up -d napcat_1    # NapCat QQ 接入服务（Bot 2）
docker-compose up -d playwright  # Playwright 浏览器服务
docker-compose up -d eriix      # EriiX 主服务
```
