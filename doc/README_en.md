...
erii is an AI group chat bot with emotions, memory, flow-state cognition, and proactive behavior. Based on a multi-dimensional psychological model, erii participates in group chats like a real person — interjecting, breaking the ice, and adjusting speaking style based on emotional state. Over long-term interaction, it learns group chat lingo, builds user profiles, and tracks trending memes. It also supports self-rule management and automatic summarization, with a CLI management tool for configuration and data management.
...
- 📜 **Self-Rule Management**: File system rule storage with multi-level scope control
- 📝 **Summary Generation**: Automatic conversation summarization with scheduled and manual triggers
