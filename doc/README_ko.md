...
| **erii-cli**       | CLI 도구           | 대화형 TUI 구성 관리 도구              |
...
Erii는 구성 관리를 위한 대화형 CLI 도구를 제공합니다:
...
- **Env Config** - 환경 변수 구성 (`.env.local`)
- **Application Config** - 애플리케이션 구성 (`application.conf`)
- **Plugin Config** - 플러그인 구성 관리
- **Souls** - 영혼 구성 브라우저
- **Rules** - 규칙 파일 브라우저

위아래 화살표로 탐색, Enter로 편집, Esc로 돌아갑니다.
...
│   └── seeddream/               # AI 이미지 생성
├── erii-cli/                    # 🖥️ CLI 도구 모듈
│   ├── cmd/                    # CLI 명령어
│   │   └── config.go           # config 명령어 - TUI 구성 관리
│   └── internal/tui/           # TUI 컴포넌트
│       ├── components/           # TUI 컴포넌트
│       │   ├── config/         # 구성 편집기
│       │   ├── md/             # Markdown 브라우저
│       │   └── plugin/         # 플러그인 브라우저
