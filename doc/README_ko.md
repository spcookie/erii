...
erii는 감정, 기억, 흐름 기반 사고, 능동적 행동을 갖춘 AI 그룹 채팅 봇입니다. 다차원 심리 모델을 기반으로, erii는 실제 사람처럼 그룹 채팅에 참여하여 대화에 끼어들고, 분위기를 깨뜨리며, 감정 상태에 따라 말투를 조정합니다. 장기적인 상호작용을 통해 그룹 채팅 용어를 학습하고, 사용자 프로필을 구축하며, 유행하는 밈을 추적합니다. 또한 자기 규칙 관리와 자동 요약 생성, CLI 관리 도구를 통한 설정 및 데이터 관리를 지원합니다.
...
- ⚡ **능동적 행동**: 충동 값 기반 대화 끼어들기, 분위기 깨기, 일상 상호작용
- 📜 **자기 규칙 관리**: 파일 시스템 규칙 저장, 다단계 범위 제어
- 📝 **요약 생성**: 대화 요약 자동 생성, 예약 및 수동 트리거 지원
