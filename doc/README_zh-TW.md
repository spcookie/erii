...
繁體中文 | [English](../README.md) | [中文](./README_zh.md) | [한국어](./README_ko.md)
...
[![Bubble Tea](https://img.shields.io/badge/Bubble_Tea-TUI-2C3E50?style=for-the-badge&logo=bubble-tea&logoColor=white)](https://github.com/charmbracelet/bubbletea)
...
- **重複任務** - 支援 Cron 表達式定義重複提醒
- **作用域管理** - Bot/Group 級別隔離
...
| **animal**         | AgentExtension   | 虛擬寵物養殖插件，角色扮演與好感度系統             |
| **erii-cli**       | CLI 工具           | 互動式 TUI 配置管理工具                  |
...
- **Env Config** - 環境變數設定（`.env.local`）
- **Application Config** - 應用程式設定（`application.conf`）
- **Plugin Config** - 插件設定管理
- **Souls** - 靈魂設定瀏覽
- **Rules** - 規則檔案瀏覽

使用上下鍵導航，Enter 進入編輯，Esc 返回。
...
├── erii-cli/                    # 🖥️ CLI 工具模組
│   ├── cmd/                    # CLI 命令
│   │   └── config.go           # config 命令 - TUI 設定管理
│   └── internal/tui/           # TUI 介面元件
│       ├── components/         # TUI 元件
│       │   ├── config/         # 設定編輯器
│       │   ├── md/             # Markdown 瀏覽器
│       │   └── plugin/         # 插件瀏覽器
