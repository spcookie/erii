...
class MinimaxVision : IVision {
    ...
    override suspend fun vision(prompt: String, url: String): String {
        ...
        val node: JsonNode = httpClient.post(baseUrl) {
            headers {
                ...
                append("MM-API-Source", "Minimax-MCP")
            }
            setBody(
                mapOf(
                    "prompt" to prompt,
        ...
        if (!node.has("content")) {
            throw MinimaxRequestError("Response missing content: $node")
        }
    ...
    private suspend fun downloadFromUrl(imageUrl: String): String {
        return runCatching {
            ...
            throw MinimaxRequestError("Failed to download image from URL: ${e.message}")
        }
    }
    ...
    private fun readFromFile(imageUrl: String): String {
        ...
        if (!file.exists()) {
            throw MinimaxRequestError("Local image file does not exist: $imageUrl")
        }
        return try {
            ...
            throw MinimaxRequestError("Failed to read local image file: ${e.message}")
        }
    }
}

class MinimaxRequestError(message: String) : Exception(message)
